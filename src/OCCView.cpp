...
void OCCT::handleWindowChanged(QQuickWindow *win) {
  if (win) {
    ...
    //! [1]
    //! [3]
    // Ensure we start with cleared to black. The OCCT's blend mode relies on
    // this.
...
void OCCTRenderer::initViewer(Aspect_Drawable drawable) {
  if (!m_viewer) {
    ...
    Handle(OpenGl_GraphicDriver) driver =
        new OpenGl_GraphicDriver(display, Standard_False);
    ...
    m_view->SetWindow(window, reinterpret_cast<Aspect_RenderingContext>(
                                  wglGetCurrentContext()));

    // m_view->SetWindow(window);
    //  m_view->TriedronDisplay(Aspect_TOTP_RIGHT_LOWER,
    //  Quantity_NOC_WHITESMOKE, 0.2, V3d_ZBUFFER);}
  }
}
//! [6]
...
void OCCTRenderer::init() {
  ...
  Handle(WNT_Window) window =
      new WNT_Window(Aspect_Drawable(m_window->winId()));
  m_view = m_viewer->CreateView();
...
void OCCTRenderer::paint() {
  //    m_view->SetImmediateUpdate(Standard_False);
  //           m_view->SetWindow(window,
  //           reinterpret_cast<Aspect_RenderingContext>(renderContext));
  //           m_view->TriedronDisplay(Aspect_TOTP_RIGHT_LOWER,
  //           Quantity_NOC_WHITESMOKE, 0.1, V3d_ZBUFFER);
  gp_Ax2 axis;
  axis.SetLocation(gp_Pnt(0.0, 10.0, 0.0));
